...
private[cluster] case class Gossip(
  ...
  def leader: Option[Address] = leader(members)

  def roleLeader(role: String): Option[Address] = leader(members.filter(_.hasRole(role)))

  private def leader(mbrs: immutable.SortedSet[Member]): Option[Address] = {
